...
CREATE TABLE SUPPLYCHAIN.AUDITLOGFACT (
  ...
  USER_ID string NOT NULL COMMENT 'Unique identifier for each user in the CRM. This ID is used as the primary key for linking user records throughout the system.',
  CONSTRAINT FK_18 FOREIGN KEY (USERID) REFERENCES SUPPLYCHAIN.USERDIM (USERID),
  CONSTRAINT FK_122 FOREIGN KEY (USER_ID) REFERENCES CRM.USER (USER_ID)
);