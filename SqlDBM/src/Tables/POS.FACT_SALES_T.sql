...
CREATE TABLE POS.FACT_SALES_T (
  ...
  CUSS_KEY bigint COMMENT 'Foreign key to DIM_CUSTOMER; NULL for anonymous/cash transactions',
  ...
  CUSS_ID decimal(10,0) NOT NULL COMMENT 'A unique identifier for each customer.',
  FROM_DTS timestamp NOT NULL,
  CONSTRAINT FK_99 FOREIGN KEY (DATE_KEY) REFERENCES POS.DIM_DATE_T (DATE_KEY),
  CONSTRAINT FK_100 FOREIGN KEY (STORE_KEY) REFERENCES POS.DIM_STORE_T (STORE_KEY),
  CONSTRAINT FK_101 FOREIGN KEY (PRODUCT_KEY) REFERENCES POS.DIM_PRODUCT_T (PRODUCT_KEY),
  CONSTRAINT FK_102 FOREIGN KEY (CUSS_ID, FROM_DTS) REFERENCES POS.DIM_CUSS_T (CUSS_ID, FROM_DTS)
)
