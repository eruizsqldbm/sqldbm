-- ************************** SqlDBM: BigQuery **************************
-- ***** Generated by SqlDBM: GBQ Demo1 by eric.ruiz+yew@sqldbm.com *****

-- ************************************** ERP.LFA_1
CREATE TABLE ERP.LFA_1
(
LIFNR  string NOT NULL PRIMARY KEY NOT ENFORCED OPTIONS(description = 'Unique identifier for the vendor; primary key of the table.'),
NAME_1 string(50) OPTIONS(description = 'Primary name of the vendor.'),
LAND_1 string OPTIONS(description = 'Specifies the country of the vendor; linked to table T005 for country codes.'),
ORT_01 string(35) OPTIONS(description = 'City where the vendor is located.'),
PSTLZ  string OPTIONS(description = 'Postal code for the vendors address.'),
TELF_1 string OPTIONS(description = 'Primary contact telephone number for the vendor.')
)
OPTIONS(description = 'Table stores general information about vendors (suppliers), including their names, addresses, and contact details.',
friendly_name = 'Vendor Master Data');
