...
CREATE TABLE SC.PRODUCTION_ORDER_T (
  ...
  CAMPAIGN_HKEY binary NOT NULL,
  CONSTRAINT MATERIAL_ID FOREIGN KEY (MATERIAL_ID) REFERENCES SC.MATERIAL_T (MATERIAL_ID),
  CONSTRAINT PLANT_ID FOREIGN KEY (PLANT_ID) REFERENCES SC.PLANT_T (PLANT_ID),
  CONSTRAINT FK_103 FOREIGN KEY (CAMPAIGN_HKEY) REFERENCES sales_infomart.dim_campaign (campaign_hkey)
)
