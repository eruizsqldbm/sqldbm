-- ************************** SqlDBM: BigQuery **************************
-- ***** Generated by SqlDBM: GBQ Demo1 by eric.ruiz+yew@sqldbm.com *****

-- ************************************** CRM.ROLE
CREATE TABLE CRM.ROLE
(
ROLE_ID      string NOT NULL PRIMARY KEY NOT ENFORCED OPTIONS(description = 'Unique identifier for each role, used as the primary key.'),
NAME         string(80) NOT NULL OPTIONS(description = 'Specifies the name of the role, such as Sales Manager or Support Agent.'),
CREATED_DATE date OPTIONS(description = 'Records when the role was created in the CRM system.')
)
OPTIONS(description = 'Defines user roles in the CRM system, enabling role-based access control and hierarchical organization.',
friendly_name = 'Role');
